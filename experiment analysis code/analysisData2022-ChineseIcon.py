...
def drawBuildResultUseSns():
    ...
    for i in range(0, len(file_path_list)):
        ...
        # 绘制节点数据-资源数量关系图
        subplot2[int(i / 2)].plot(node_count, tokenID, color=colorlist[int(i % 2)], alpha=0.8, linestyle=linestylelist[int(i%2)])
    ...
    ax3.fill_between(node_count_list[1], tokenID_list[1], 0,  # 上限，下限
                     ...
                     alpha=0.2)  # 透明度
    ax3.set_xlabel('FBSI节点数(正态分布)', size=frontsize)
    ax3.set_ylabel('空间-文本数据量', size=frontsize) # 'Number of Resources'
    ...
    ax4.fill_between(node_count_list[3], tokenID_list[3], 0,  # 上限，下限
                     facecolor=colorlist[2],  # 填充颜色
                     alpha=0.2)  # 透明度
    ax4.set_xlabel('FBSI节点数(均匀分布)', size=frontsize)
    ax4.set_ylabel('空间-文本数据量', size=frontsize)# 'Number of Resources'
...
def drawCalculatedRatioOfLeafNodeUseSns():
    ...
    font2 = {'size': 18}
    legend = ['FBSI', '四叉树', 'FBSI', 'FBSI', '四叉树', '四叉树']
    color = [(31 / 255, 50 / 255, 255 / 255), (255 / 255, 100 / 255, 14 / 255)]
    ...
    ax1.set_xlabel('叶节点数据容量负载(%) (正态分布)', font1)
    ax1.set_ylabel('叶节点占比(%)', font1)
    ...
    ax2.set_xlabel('叶节点数据容量负载(%) (均匀分布)', font1)
    ax2.set_ylabel('叶节点占比(%)', font1)
